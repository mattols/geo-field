...
4. Imagine you're working on two field projects in UTM Zone 12N. In both cases, you're walking along an east-west path that is 1 km long, counting the number of Great Basin Sagebrush plants within 10 meters of your path. The first project is located at an easting of 500,000 meters (center of UTM Zone). The second project is located at an easting of 750,000 meters (closer to the edge of the zone, where distortion is more noticeable). You're trying to understand whether the distortion will significantly affect your measurements. What would the apparent plant density be at the edge compared to the center due to distortion from the projection alone?
      * *Hint: compare the ratios of* ${plants}/{true~distance}$ *between locations*
...
8. Calculate the linear distance (*Euclidean distance in our case*) between the center points and the 2nd, 3rd and 9th coordinate pairs in `p1` (unit should be meters). How do these distances compare with your standard deviation listed above?
      * *Hint: convert x and y degree differences to meters prior to calculating euclidean distance*
      
9. Create a basic scatterplot in R using the base `plot` function to show the GPS measurements. Add a center point and buffer for the first and second standard deviation around your center point. Note how this differs in the x and y direction. Follow these steps and use the code below:
...
# ...add your code here (copy/modify ellipse equations above)
```
...
# ^...update names for 'x_differences' and 'y_differences' based on your data
...
# add a legend (optional) - '\u03C3' is character for sigma
# ...
...
p1_nad83 <- project(p1_nad83, 'epsg:4326')
...
11. Use the provided table of GPS measurements from a previous year to calculate the center point for each corner of the ponds (P1-5), calculate the distances between each point and use these to estimate the dimensions of the pond. How close to the truth do you think these results are? (consider the standard deviation of each corner) 
        ...
        * In R, you can subset the data with indexing or use the `aggregate()` function to do this for the entire data frame, specifying the relationship `cbind(Latitude, Longitude) ~ Title, data = ..., FUN = ...` *see help doc for more info*
        * (optional) Use these points to create a polygon and estimate the area. Could also compare with basemaps. 


12. Follow along on Dr. Bunds' instructions in the attached document to explore epochs. List your coordinates from the two separate reference frames and list the distance between the two points and calculate the offset between coordinates in meters.
...
**Note:** *basic R functionality is found in later sections.*
